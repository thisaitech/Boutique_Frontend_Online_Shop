...
// Women categories and filter options
...
const materials = ['Silk', 'Cotton', 'Georgette', 'Chiffon', 'Velvet', 'Net', 'Satin']
...
function Women() {
  ...
  const maxPrice = Math.max(...womenProducts.map(p => p.price), 60000)

  // Update price range when products change
  useEffect(() => {
    if (womenProducts.length > 0) {
      const max = Math.max(...womenProducts.map(p => p.price), 60000)
      setPriceRange([0, max])
    }
  ...
  const selectedCategoryInfo = womenCategories.find(cat => cat.id === selectedCategory)

  return (
    <div className="shop-page">
      {/* Dynamic Promo Section - Admin Controlled */}
      <PromoSection category="women" />

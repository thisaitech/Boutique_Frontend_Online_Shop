...
// Fashion categories and filter options
...
const colors = ['Red', 'Blue', 'Green', 'Pink', 'Yellow', 'Purple', 'Black', 'White', 'Gold', 'Silver', 'Brown']
const materials = ['Leather', 'Canvas', 'Metal', 'Fabric', 'Velvet', 'Satin']
...
function Fashion() {
  ...
  const maxPrice = Math.max(...fashionProducts.map(p => p.price), 50000)

  // Update price range when products change
  useEffect(() => {
    if (fashionProducts.length > 0) {
      const max = Math.max(...fashionProducts.map(p => p.price), 50000)
      setPriceRange([0, max])
    }
  ...
  const selectedCategoryInfo = fashionCategories.find(cat => cat.id === selectedCategory)

  return (
    <div className="shop-page fashion-page-theme">
      {/* Dynamic Promo Section - Admin Controlled */}
      <PromoSection category="fashion" />
      ...
      <div className="mobile-quick-nav">
        <div className="mobile-quick-nav-inner">
          ...
          <Link to="/kids" className="btn btn-outline">Kids</Link>
        </div>
      </div>
            ...
            <div className="products-container with-sidebar">
              {!hasAnyProducts ? (
                ...
                >
                  ...
                  <p>Fashion accessories collection coming soon! Stay tuned for beautiful ornaments and bags.</p>

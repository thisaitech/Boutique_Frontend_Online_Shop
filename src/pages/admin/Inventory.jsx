...
import { FiPlus, FiEdit2, FiTrash2, FiX, FiSearch, FiImage, FiStar, FiTrendingUp, FiDroplet, FiChevronLeft, FiChevronRight, FiEye, FiEyeOff } from 'react-icons/fi'
import {
  fetchProducts,
  selectProducts,
...
// Categories, colors, and materials - static options for product forms
const categories = [
...
const materials = ['Silk', 'Cotton', 'Georgette', 'Chiffon', 'Velvet', 'Net', 'Satin', 'Leather', 'Canvas', 'Metal', 'Fabric']
...
function Inventory() {
  ...
  // CRUD operations using Redux only
  const updateProduct = async (id, data) => {
    try {
      await dispatch(updateAdminProduct({ productId: id, productData: data })).unwrap()
  ...
  const filteredProducts = useMemo(() => {
    return inventory
      .filter(product => {
        ...
        // Check if product is out of stock (either inStock is false OR stockCount is 0)
        const isOutOfStock = !product.inStock || product.stockCount === 0
        const matchesStock = !filterOutOfStock || isOutOfStock
  ...
  }, [inventory, searchQuery, filterCategory, filterOutOfStock])
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [searchQuery, filterCategory, filterOutOfStock])
  ...
  const toggleProductVisibility = async (product) => {
    ...
    try {
      ...
      toast.error(error || 'Failed to update product visibility')
    }
  }
    ...
    <div className="inventory-page">
      ...
      <div className="page-header">
        <div className="header-left">
          ...
          <p>{inventory.length} products total</p>
        </div>
      ...
      <div className="filters-bar glass-card">
        ...
        <button
          className={`filter-btn ${filterOutOfStock ? 'active' : ''}`}
          onClick={() => setFilterOutOfStock(!filterOutOfStock)}
          title="Show only out of stock products"
        >
          <FiEyeOff />
          {filterOutOfStock ? 'Show All Stock' : 'Out of Stock Only'}
        </button>
      </div>
              ...
              <tr key={productKey}>
                ...
                <td>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    {(() => {
                      // Check if product is actually in stock based on stockCount
                      const actuallyInStock = product.inStock && (product.stockCount === undefined || product.stockCount > 0)
                      return (
                        <span className={`stock-badge ${actuallyInStock ? 'in-stock' : 'out-stock'}`}>
                          {actuallyInStock ? 'In Stock' : 'Out of Stock'}
                        </span>
                      )
                    })()}
                    {product.stockCount !== undefined && product.stockCount >= 0 && (
                      <span style={{
                        fontSize: '0.85rem',
                        color: product.stockCount === 0 ? '#e74c3c' : product.stockCount <= 3 ? '#f39c12' : 'var(--text-muted)',
                        fontWeight: product.stockCount <= 3 ? '600' : 'normal'
                      }}>
              ...
              <form onSubmit={handleSubmit} className="product-form">
                <div className="form-grid">
                  ...
                  <div className="form-group full-width">
                    ...
                    <div className="file-upload">
                      <input
                        ...
                        id="inventory-main-image-file"
                      />
                      <label htmlFor="inventory-main-image-file" className="file-label">
                  ...
                  <div className="form-group full-width">
                            ...
                            <div key={index} className="variant-image-row">
                              ...
                              <div className="file-upload compact">
                                <input
                                  ...
                                  id={`inventory-variant-file-${index}`}
                                />
                                <label htmlFor={`inventory-variant-file-${index}`} className="file-label">
                                  Camera / device
                                </label>
                              </div>
                    ...
                    <small className="size-hint">Add different color options with their images. These will appear as color swatches on product cards and in the product detail page.</small>
                  </div>
